...
__global__ void negate(int * d_a, int size) {
...
int main(int argc, char *argv[]) {
  ...
  NumberOfBlocks.x = ((ARRAY_SIZE-1)/THREADS_PER_BLOCK) + 1;
  ...
  negate<<<NumberOfBlocks,THREADS_PER_BLOCK>>>(d_a, ARRAY_SIZE);
